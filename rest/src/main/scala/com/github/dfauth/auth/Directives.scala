...
import com.github.dfauth.authzn.{ImmutableSubject, Subject}
...
import com.github.dfauth.jwt.{JWTVerifier, User, UserCtx}
...
//      xAuthCookie <- optionalCookie("X-Authorization-Token").map(_.map(_.value))
  ...
  def authenticate(verifier:JWTVerifier): Directive1[UserCtx[Subject]] = {
    var user:User = null
    bearerToken.flatMap {
      case Some(token) => {
        verifier.authenticateToken(token, verifier.asUser) match {
          case s:Success[User] => provide(new UserCtxImpl(token, toSubject(s.getPayload)))
